...
`https://blackfire.io` and go ahead and make sure that you,

let's do this.

I don't want to buy fire.io and go ahead and login or sign up. I'm going to go with
my Symfony connect account allow and
...
tab, and I'll run through tap. 

```terminal-silent
brew tap blackfireio/homebrew-blackfire
```

That's going to give me access to a, some brew
...
that's exactly what the next command is. A `sudo blackfire-agent --register`. So
...
Then spin back over again and then copy the launch control load command here. 

```terminal-silent
lauchctl load -w ~/Library/LaunchAgents/homebrew.mxcl.blackfire-agent.plist
```

Cool, so thanks to this, this Blackberry agent is
...
the PHP extension, so you can see here that they have little copy here for 
`brew install blackfire-php56`  well hopefully you're not using a PHP 5.6 anymore.
Why didn't do is actually go over here and figure out what version of PHP I am
running. So I'm gonna say 

```terminal
php --version
```

7.3.6 so I'm going to run it

```terminal
brew install blackfire-php73
```

in this the extension doesn't need any type of server ID or server token. 
It doesn't need any type of authentication information. It's just a PHP extension. 
It's going to pass the data to the agent and the agent already has a server ID and 
server tokens so we can send to us.
...
And with any luck we will now have a Blackfire extension
...
what I'm doing. `Show page initial` and hit enter. Now if you, this is where, uh,
